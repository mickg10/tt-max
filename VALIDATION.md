...
- `uv run pytest -q`: 17 passed. Includes real CPU and memory work, automatic timeout, cancellation, preflight cancellation without spawning, input/memory bounds, board power deduplication/missing values, HTTP token and cross-origin checks.
